...
import { InputAddress, InputAddressSimple, InputBalance, MarkError, MarkWarning, Modal, Toggle, TxButton } from '@polkadot/react-components';
    ...
    >
      <Modal.Content>
        <div className={className}>
          <Modal.Columns hint={'The transferred balance will be subtracted (along with fees) from the sender account.'}>
            <InputAddress
          ...
          <Modal.Columns hint={'The beneficiary will have access to the transferred fees when the transaction is included in a block.'}>
            <InputAddressSimple
              onChange={setRecipientId}
              label={<h3 className="tcm-header">To</h3>}
            />
          </Modal.Columns>
          <Modal.Columns hint={'If the recipient account is new, the balance needs to be more than the existential deposit. Likewise if the sending account balance drops below the same value, the account will be removed from the state.'}>
            {canToggleAll && isAll
              ? (
                <InputBalance
                  autoFocus
              ...
              : (
                <>
                  ...
                  <InputBalance
                    ...
                    help={'The minimum amount that an account should have to be deemed active'}
                    isDisabled
          ...
          <Modal.Columns hint={'With the keep-alive option set, the account is protected against removal due to low balances.'}>
            ...
            {canToggleAll && (
              <Toggle
                className='typeToggle'
                label={'Transfer the full account balance, reap the sender'}
                onChange={setIsAll}
                value={isAll}
              />
            )}
